...
bym_models_sp <- lapply(sim_aggregated_list_sp, function(X) {
  tryCatch({
    inla( counts ~ fac_dist + f(ID, model = "bym2", graph = g_sp),
          family = "poisson",
          data = X, E = pop_density,  control.predictor = list(compute = TRUE),
...
#Stricter priors around precision because of fewer areas
prior_list_mp <-  list(
...
bym_models_mp <- lapply(sim_aggregated_list_mp, function(X) {
  ...
  tryCatch({
    bym_mod <- inla(counts ~ fac_dist + f(ID, model = "bym2", graph = g_mp,
                                          hyper = prior.list), family = "poisson",
...
betas_sp <- lapply(bym_models_sp, function(X){
